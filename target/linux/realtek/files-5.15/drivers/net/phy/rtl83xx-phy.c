...
{
	pr_info("%s %d\n", __func__, mode);
	if ((sds_num < 0) || (sds_num >= RTL930X_SDS_PORT_CNT)) {
...
{
	pr_debug("%s: Resetting sds %d to mode 0x%x\n", __func__, sds_num, mode);
	if ((sds_num < 0) || (sds_num >= RTL930X_SDS_PORT_CNT)) {
		pr_err("Wrong SerDes number: %d\n", sds_num);
		return;
	}

	rtl9300_sds_set(sds_num, RTL930X_SDS_MODE_SEL_OFF);
	msleep(10);
...
u32 rtl9300_sds_mode_get(int sds_num)
{
	...
	return RTL930X_SDS_MODE_SEL_GET(sds_num, sw_r32(RTL930X_SDS_MODE_REG(sds_num)));
}
...
static int rtl8214qf_sds_mode_set(struct phy_device *phydev, int mode)
{
	...
	pr_debug("%s port %d, ctrl reg is 0x%x, current mode is 0x%lx\n", __func__, port, v, v & RTL8295_SDS_CTRL_REG_S0_MODE_MASK);
...
static int rtl9300_read_status(struct phy_device *phydev)
{
	...
	if (mode == RTL930X_SDS_MODE_SEL_OFF) {
		mode = rtl9300_sds_field_r(sds_num, 0x1f, 9, 11, 7);
	} else if (mode == RTL930X_SDS_MODE_SEL_10GBASER) {
	...
	if (latch_status) {
		phydev->link = true;
		if (mode == RTL930X_SDS_MODE_SEL_10GBASER) {
			phydev->speed = SPEED_10000;
...
{
	int sds_mode = RTL930X_SDS_MODE_SEL_OFF;
	int lc_value;
	...
	switch (phy_if) {
	case PHY_INTERFACE_MODE_SGMII:
		sds_mode = RTL930X_SDS_MODE_SEL_SGMII;
		lc_on = false;
		lc_value = 0x1;
		break;

	case PHY_INTERFACE_MODE_HSGMII:
		sds_mode = RTL930X_SDS_MODE_SEL_HISGMII;
		lc_value = 0x3;
	...
	case PHY_INTERFACE_MODE_1000BASEX:
		sds_mode = RTL930X_SDS_MODE_SEL_1000BASEX;
		lc_on = false;
		break;

	case PHY_INTERFACE_MODE_2500BASEX:
		sds_mode = RTL930X_SDS_MODE_SEL_2500BASEX;
		lc_value = 0x3;
	...
	case PHY_INTERFACE_MODE_10GBASER:
		sds_mode = RTL930X_SDS_MODE_SEL_10GBASER;
		lc_on = true;
		lc_value = 0x5;
		break;

	case PHY_INTERFACE_MODE_NA:
		sds_mode = RTL930X_SDS_MODE_SEL_OFF;
		break;
...
{
	const struct sds_tx_eye_param *sds_eye = NULL;
	int page;
	...
	case PHY_INTERFACE_MODE_USXGMII:
		...
		break;
	default:
		pr_debug("%s: No needed configuration for PHY mode %s\n", __func__, phy_modes(phy_if));
		return;
	}
	...
	pr_debug("%s SerDes %d, pre-amp 0x%02x, post-amp 0x%02x, pre-en 0x%02x, post-en enable 0x%02x, main-amp val 0x%02x, impedance 0x%02x\n",
...
void rtl9300_do_rx_calibration(int sds, phy_interface_t phy_mode)
{
	...
	/* Do this only for 10GR mode, SDS active in mode RTL930X_SDS_MODE_SEL_10GBASER */
	if (rtl9300_sds_field_r(sds, 0x1f, 9, 11, 7) == RTL930X_SDS_MODE_SEL_10GBASER) {
...
int rtl9300_serdes_setup(int sds_num, phy_interface_t phy_mode)
{
	...
	case PHY_INTERFACE_MODE_1000BASEX:
		...
		break;
	case PHY_INTERFACE_MODE_XGMII:
		sds_mode = RTL930X_SDS_MODE_SEL_XSGMII;
		break;
	case PHY_INTERFACE_MODE_10GBASER:
		sds_mode = RTL930X_SDS_MODE_SEL_10GBASER;
		break;
	case PHY_INTERFACE_MODE_USXGMII:
		sds_mode = RTL930X_SDS_MODE_SEL_USXGMII;
		break;
	default:
	...
	/* Set Serdes Mode */
	rtl9300_sds_set(sds_num, RTL930X_SDS_MODE_SEL_10GBASER); /* RTL930X_SDS_MODE_SEL_10GKR_1000BASEX_AUTO */
...
int rtl9300_configure_8218d(struct phy_device *phydev)
{
	...
	pr_info("%s: A Port %d, SerDes is %d phy-mode %s port %d\n", __func__, phy_addr, sds_num, phy_modes(phydev->interface), phydev->port);
	switch (phydev->interface) {
	case PHY_INTERFACE_MODE_QSGMII:
		sds_mode = RTL930X_SDS_MODE_SEL_QSGMII;
		break;
	case PHY_INTERFACE_MODE_XGMII:
		sds_mode = RTL930X_SDS_MODE_SEL_XSGMII;
		break;
	default:
		pr_err("%s: unsupported serdes mode\n", __func__);
		return -EINVAL;
	}
	...
	/* phy_addr matches port in this particular case */
	sw_w32_mask(0, RTL930X_MAC_FORCE_MODE_CTRL_EN, RTL930X_MAC_FORCE_MODE_CTRL_REG(phy_addr));
	msleep(20);
	...
	rtl9300_rtl8218d_phy_setup(phydev, phydev->interface);
	...
	rtl9300_sds_tx_config(sds_num, phydev->interface);
 ...
 */
sds_config rtl9300_rtl8226_a_sds_10g_hisgmii_lane0[] = {
	{ 0x00, 0x0e, 0x3053 }, { 0x01, 0x14, 0x0100 }, { 0x21, 0x03, 0x8206 },
	{ 0x21, 0x05, 0x40b0 }, { 0x21, 0x06, 0x0010 }, { 0x21, 0x07, 0xf09f },
	{ 0x21, 0x0c, 0x0007 }, { 0x21, 0x0d, 0x6009 }, { 0x21, 0x0e, 0x0000 },
	{ 0x21, 0x0f, 0x0008 }, { 0x24, 0x00, 0x0668 }, { 0x24, 0x02, 0xd020 },
 ...
 * the RTL8226 PHY as found in a Zyxel XGS1210
 */
sds_config rtl9300_rtl8226_a_sds_10g_hisgmii_lane1[] = {
	{ 0x00, 0x0e, 0x3053 }, { 0x01, 0x14, 0x0100 }, { 0x21, 0x03, 0x8206 },
	{ 0x21, 0x06, 0x0010 }, { 0x21, 0x07, 0xf09f }, { 0x21, 0x0a, 0x0003 },
	{ 0x21, 0x0b, 0x0005 }, { 0x21, 0x0c, 0x0007 }, { 0x21, 0x0d, 0x6009 },
	{ 0x21, 0x0e, 0x0000 }, { 0x21, 0x0f, 0x0008 }, { 0x24, 0x00, 0x0668 },
...
int rtl9300_configure_rtl8226(struct phy_device *phydev)
{
	...
	if (sds_num % 2) {
		rtl9300_sds_patch(sds_num, rtl9300_rtl8226_a_sds_10g_hisgmii_lane1,
				  sizeof(rtl9300_rtl8226_a_sds_10g_hisgmii_lane1) / sizeof(sds_config));
	} else {
		rtl9300_sds_patch(sds_num, rtl9300_rtl8226_a_sds_10g_hisgmii_lane0,
				  sizeof(rtl9300_rtl8226_a_sds_10g_hisgmii_lane0) / sizeof(sds_config));
	}
...
int rtl9300_rtl8226_mode_set(int port, int sds_num, phy_interface_t phy_mode)
{
	...
	sw_w32_mask(0, RTL930X_MAC_FORCE_MODE_CTRL_EN, RTL930X_MAC_FORCE_MODE_CTRL_REG(port));
	msleep(20);
